...
wd<-getwd()
wd<-"E:/Intersection of Social Vulnerability and Flood/Run scripts"
source(paste0(wd,'/pca_kn_new.R'))
...
files<-list.files(path= paste0(wd,"/census tract data by states/"), pattern= '_Final_Table') #get list of intersection files
...
files<-list.files(path= paste0(wd,"/block group data by states/"), pattern= '_Final_Table') #get list of intersection files
